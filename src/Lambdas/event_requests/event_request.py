...
class EventRequest:
    ...
    def _validate_unexpected_fields(cls, cls_fields:set, kwargs:set)->None:
        ...
        if unexpected_fields:
            raise RequestException(f'unexpected fields were found on the request :{list(unexpected_fields)}', 400) 
    
    @classmethod
    def _validate_missing_fields(cls, required_fields:set, kwargs:set)->None:
        missing_fields = required_fields.difference(kwargs)
        if missing_fields:
            raise RequestException(f'Missing fields were found on the request: {list(missing_fields)}', 400)
